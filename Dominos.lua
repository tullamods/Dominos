...
function Dominos:OnCmd(msg)
	local args = getArgs(string.lower(msg)) or msg
	local cmd1, cmd2 = unpack(args)
	--frame functions
	if cmd1 == 'config' or cmd1 == 'lock' then
		self:ToggleLockedFrames()
	elseif cmd1 == 'scale' then
		self:ScaleFrames(cmd2)
	elseif cmd1 == 'setalpha' then
		self:SetOpacityForFrames(cmd2)
	elseif cmd1 == 'fade' then
		self:SetFadeForFrames(cmd2)
	elseif cmd1 == 'setcols' then
		self:SetColumnsForFrames(cmd2)
	elseif cmd1 == 'pad' then
		self:SetPaddingForFrames(cmd2)
	elseif cmd1 == 'space' then
		self:SetSpacingForFrame(cmd2)
	elseif cmd1 == 'show' then
		self:ShowFrames(cmd2)
	elseif cmd1 == 'hide' then
		self:HideFrames(cmd2)
	elseif cmd1 == 'toggle' then
		self:ToggleFrames(cmd2)
	--actionbar functions
	elseif cmd1 == 'numbars' then
		self:SetNumBars(tonumber(cmd2))
	elseif cmd1 == 'numbuttons' then
		self:SetNumButtons(tonumber(cmd2))
	--profile functions
